...
%%%
%%% @end
%%% Created : 15. Apr 2017 1:26 PM
%%%-------------------------------------------------------------------
-module(sup_frequency_server).
-author("Rakkatakka").
...
-spec allocate(Server) -> allocate_operation_result() | error_timeout() when
    Server::atom().
allocate(Server) ->
    send_client_message(Server, {request, self(), allocate}).

-spec deallocate(Server, Freq) -> deallocate_operation_result() | error_timeout() when
    Server::atom(),
    Freq::freq().
deallocate(Server, Freq) ->
    send_client_message(Server, {request, self(), {deallocate, Freq}}).

-spec stop(Server) -> stopped when
    Server::atom().
stop(Server) ->
    send_client_message(Server, {request, self(), stop}).

send_client_message(Server, Msg) ->
    clear(),
...
-spec loop(SupPid, State) -> server_result() when
    SupPid::pid(),
    State::server_state().
loop(SupPid, State) ->
    receive
        ...
        {request, Pid, stop} ->
            Pid ! {reply, stopped};
        {'EXIT', SupPid, _Reason} ->
            io:format("Supervisor ~w died, going down myself ~w ~n", [SupPid, self()]),
            exit(self(), kill);
        {'EXIT', Pid, _Reason} ->
